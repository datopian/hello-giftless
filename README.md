...
| Token | Replaced with |
|-------|--------------|
| `Hello Giftless` | Human-readable portal name |
| `hello-giftless` | URL-safe slug |
| `A demo data portal with large data versioned in git and served from Cloudflare R2 via Giftless.` | One-sentence portal description |
...
The navbar's brand text uses the same `Hello Giftless` token, so it is already set to your
